...
#define RT305X_ESW_REG_RXPC(_x)		(0xe8 + (4 * _x))
#define RT305X_ESW_REG_P1PC		0xec
#define RT305X_ESW_REG_P2PC		0xf0
#define RT305X_ESW_REG_P3PC		0xf4
#define RT305X_ESW_REG_P4PC		0xf8
#define RT305X_ESW_REG_P5PC		0xfc
#define RT305X_ESW_REG_TXPC(_x)		(0x150 + (4 * _x))
...
#define RT305X_ESW_PCR0_RD_PHY_CMD	BIT(14)
#define RT305X_ESW_PCR0_WT_PHY_CMD	BIT(13)
#define RT305X_ESW_PCR0_CPU_PHY_REG_S	8

#define RT305X_ESW_PCR1_WT_DONE		BIT(0)
#define RT305X_ESW_PCR1_RD_DONE		BIT(1)
...
#define RT305X_ESW_PMAP_MASK		0x3f
...
/* Port attribute strings */

#define RT305X_ESW_DUPLEX_FULL_STR "full"
#define RT305X_ESW_DUPLEX_HALF_STR "half"

#define RT305X_ESW_ANEG_ON_STR	"on"
#define RT305X_ESW_ANEG_OFF_STR "off"

#define ADVERT_NOTHING	    0
#define ADVERT_10_H	    1
#define ADVERT_10_F	    2
#define ADVERT_10_HF	    3
#define ADVERT_100_H	    4
#define ADVERT_10_H_100_H   5
#define ADVERT_10_F_100_H   6
#define ADVERT_10_HF_100_H  7
#define ADVERT_100_F	    8
#define ADVERT_10_H_100_F   9
#define ADVERT_10_F_100_F   10
#define ADVERT_10_HF_100_F  11
#define ADVERT_100_HF	    12
#define ADVERT_10_H_100_HF  13
#define ADVERT_10_F_100_HF  14
#define ADVERT_10_HF_100_HF 15
#define ADVERT_10_HF_100_HF_1000_HF 47

#define PRINT_ADVERTISE_10HALF	0x1
...
enum {
	...
	RT5350_ESW_ATTR_PORT_SPEED,
	RT5350_ESW_ATTR_PORT_DUPLEX,
	RT5350_ESW_ATTR_PORT_ANEG,
	RT5350_ESW_ATTR_PORT_ADVERT,
};
...
out:
	if (ret)
		dev_err(esw->dev, "ramips_eth: MDIO write timeout\n");
	return ret;
}

static u32 rt305x_mii_read(struct rt305x_esw *esw, u32 phy_addr, u32 phy_register)
{
	unsigned long t_start = jiffies;
	u32 ret		      = 0;

	while (1) {
		if (!(esw_r32(esw, RT305X_ESW_REG_PCR1) & RT305X_ESW_PCR1_RD_DONE))
			break;
		if (time_after(jiffies, t_start + RT305X_ESW_PHY_TIMEOUT)) {
			ret = 1;
			goto out;
	...
	esw_w32(esw,
		(phy_register << RT305X_ESW_PCR0_CPU_PHY_REG_S) | (phy_addr) | RT305X_ESW_PCR0_RD_PHY_CMD,
		RT305X_ESW_REG_PCR0);

	t_start = jiffies;
	while (1) {
		ret = esw_r32(esw, RT305X_ESW_REG_PCR1);
		if (ret & RT305X_ESW_PCR1_RD_DONE)
...
out:
	if (ret)
		dev_err(esw->dev, "ramips_eth: MDIO read timeout\n");
	return ret;
}
...
static u32 esw_get_link_status(struct rt305x_esw *esw)
{
...
static void esw_apply_hw_cfg(struct rt305x_esw *esw, uint32_t slp)
{
	int i = 0;
...
static int
rt3050_sw_delay_reset(struct switch_dev *dev, const struct switch_attr *attr,
...
static int
rt3050_wait_bit(struct rt305x_esw *esw, int reg, u32 mask, u32 val)
{
	int timeout = 120;
	u32 t;
	...
	pr_err("rt3050: timeout for reg %08x: %08x & %08x != %08x\n",
	       (unsigned int)reg, t, mask, val);
	return -ETIMEDOUT;
}

static int
rt3050_arl_dump(struct rt305x_esw *esw)
{
	int len = 0;
	int entry_len = 0;

	memset(esw->buf, 0, sizeof(esw->buf));
	...
	for(;;) {
		...
		//read MAC, VID, portmap
		regval = esw_r32(esw, RT305X_ESW_REG_ATS0);
		portmap = (regval >> 12) & 0x7F;
		vid = (regval >> 7) & 0x0F;
		age = (regval >> 4) & 0x07;
		...
		len += snprintf(esw->buf + len, sizeof(esw->buf) - len, "MAC: %02x:%02x:%02x:%02x:%02x:%02x ", 
			(mac2 >> 24) & 0xFF, (mac2 >> 16) & 0xFF, (mac2 >> 8) & 0xFF, 
			(mac2) & 0xFF, (mac1 >> 8) & 0xFF, (mac1) & 0xFF);
		len += snprintf(esw->buf + len, sizeof(esw->buf) - len, "PORTMAP: 0x%02X ", portmap);
		len += snprintf(esw->buf + len, sizeof(esw->buf) - len, "VID: 0x%02X ", vid);
		len += snprintf(esw->buf + len, sizeof(esw->buf) - len, "STATUS: 0x%02X\n", age);

		if (entry_len == 0) {
			entry_len = len;
		}
		if (sizeof(esw->buf) - len <= entry_len) {
			break;
		}
	}
...
static int
rt3050_sw_arl_dump(struct switch_dev *dev,
		    const struct switch_attr *attr,
		    struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);

	int len = rt3050_arl_dump(esw);
	if (len < 0) {
		return -EINVAL;
	}
	val->value.s = esw->buf;
...
static int esw_set_port_link(struct switch_dev *dev, int port, struct switch_port_link *link)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	u32 reg_bmcr;
	...
	reg_bmcr   = rt305x_mii_read(esw, port, MII_BMCR);

	switch (link->speed) {
	case SWITCH_PORT_SPEED_10:
		reg_bmcr &= ~BMCR_SPEED100;
		break;
	...
	if (link->duplex) {
		reg_bmcr |= BMCR_FULLDPLX | BMCR_ANRESTART;
	} else {
		reg_bmcr &= ~BMCR_FULLDPLX;
	...
	rt305x_mii_write(esw, port, MII_BMCR, reg_bmcr);
				 ...
				 struct switch_val *val)
{
	...
	if (idx < 0 || idx >= RT305X_ESW_NUM_LANWAN)
		return -EINVAL;
	reg = esw_r32(esw, RT305X_ESW_REG_RXPC(idx));
...
static int esw_get_port_speed(struct switch_dev *dev, const struct switch_attr *attr, struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	int port		       = val->port_vlan;
	u32 speed, reg;

	if (port < 0 || port >= RT305X_ESW_NUM_LAN)
		return -EINVAL;

	reg = esw_r32(esw, RT305X_ESW_REG_POA) >> port;

	if (port < RT305X_ESW_NUM_LEDS) {
		speed = (reg >> RT305X_ESW_SPD_S) & 1;
	} else {
		if (port == RT305X_ESW_NUM_PORTS - 1) {
			reg >>= 1;
		}
		speed = (reg >> RT305X_ESW_SPD_S) & 3;
	}

	switch (speed) {
	case 0:
		val->value.i = SWITCH_PORT_SPEED_10;
		break;
	case 1:
		val->value.i = SWITCH_PORT_SPEED_100;
		break;
	case 2:
	case 3:
		val->value.i = SWITCH_PORT_SPEED_1000;
		break;
	default:
		val->value.i = SWITCH_PORT_SPEED_UNKNOWN;
		break;
	}
...
static int esw_get_port_duplex(struct switch_dev *dev, const struct switch_attr *attr, struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	int idx		       = val->port_vlan;
	u32 reg;
	u8 duplex;
	if (idx < 0 || idx >= RT305X_ESW_NUM_LAN)
		return -EINVAL;

	reg = esw_r32(esw, RT305X_ESW_REG_POA) >> idx;

	duplex = (reg >> RT305X_ESW_DUPLEX_S) & 1;
	if (duplex) {
		val->value.s = RT305X_ESW_DUPLEX_FULL_STR;
		val->len     = sizeof(RT305X_ESW_DUPLEX_FULL_STR);
	} else {
		val->value.s = RT305X_ESW_DUPLEX_HALF_STR;
		val->len     = sizeof(RT305X_ESW_DUPLEX_HALF_STR);
	}

	return 0;
}

static int esw_get_port_auto_negotiation(struct switch_dev *dev, const struct switch_attr *attr,
					 struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	int idx		       = val->port_vlan;
	...
	reg = rt305x_mii_read(esw, idx, MII_BMCR);
	if (reg & BMCR_ANENABLE) {
		val->value.s = RT305X_ESW_ANEG_ON_STR;
		val->len     = sizeof(RT305X_ESW_ANEG_ON_STR);
	} else {
		val->value.s = RT305X_ESW_ANEG_OFF_STR;
		val->len     = sizeof(RT305X_ESW_ANEG_OFF_STR);
	}

	return 0;
}

static int esw_get_port_speed_advertisement(struct switch_dev *dev, const struct switch_attr *attr,
					    struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	int idx		       = val->port_vlan;
	...
	reg_advert100 = rt305x_mii_read(esw, idx, MII_ADVERTISE);
	...
	val->len     = snprintf(esw->buf2, sizeof(esw->buf2), "0x%x", advert);
	val->value.s = esw->buf2;

	return 0;
}

static int esw_set_port_speed_advertisement(struct switch_dev *dev, const struct switch_attr *attr,
					    struct switch_val *val)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);
	int idx		       = val->port_vlan;
	int ret;
	u32 reg_advert, reg_bmcr;
	long advert;

	if (idx < 0 || idx >= RT305X_ESW_NUM_LAN)
		return -EINVAL;

	reg_advert = rt305x_mii_read(esw, idx, MII_ADVERTISE);
	reg_bmcr   = rt305x_mii_read(esw, idx, MII_BMCR);

	reg_advert &= ~(ADVERTISE_10HALF | ADVERTISE_10FULL | ADVERTISE_100HALF | ADVERTISE_100FULL);
	reg_bmcr |= BMCR_ANRESTART;

	if (!strstr(val->value.s, "0x") || (strlen(val->value.s) != 4)) {
		return -EINVAL;
	}
	ret = kstrtol(val->value.s, 16, &advert);
	if (ret)
		return ret;
	...
	case ADVERT_NOTHING:
		break;
	case ADVERT_10_H:
		reg_advert |= ADVERTISE_10HALF;
		break;
	case ADVERT_10_F:
		reg_advert |= ADVERTISE_10FULL;
		break;
	case ADVERT_10_HF:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_10FULL;
		break;
	case ADVERT_100_H:
		reg_advert |= ADVERTISE_100HALF;
		break;
	case ADVERT_10_H_100_H:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_100HALF;
		break;
	case ADVERT_10_F_100_H:
		reg_advert |= ADVERTISE_10FULL | ADVERTISE_100HALF;
		break;
	case ADVERT_10_HF_100_H:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_10FULL | ADVERTISE_100HALF;
		break;
	case ADVERT_100_F:
		reg_advert |= ADVERTISE_100FULL;
		break;
	case ADVERT_10_H_100_F:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_100FULL;
		break;
	case ADVERT_10_F_100_F:
		reg_advert |= ADVERTISE_10FULL | ADVERTISE_100FULL;
		break;
	case ADVERT_10_HF_100_F:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_10FULL | ADVERTISE_100FULL;
		break;
	case ADVERT_100_HF:
		reg_advert |= ADVERTISE_100HALF | ADVERTISE_100FULL;
		break;
	case ADVERT_10_H_100_HF:
		reg_advert |= ADVERTISE_10HALF | ADVERTISE_100HALF | ADVERTISE_100FULL;
		break;
	case ADVERT_10_F_100_HF:
		reg_advert |= ADVERTISE_10FULL | ADVERTISE_100HALF | ADVERTISE_100FULL;
		break;
	...
	rt305x_mii_write(esw, val->port_vlan, MII_ADVERTISE, reg_advert);
	rt305x_mii_write(esw, val->port_vlan, MII_BMCR, reg_bmcr);
...
static const struct switch_attr esw_port[] = {
	...
	},
	{
		...
		.id	     = RT5350_ESW_ATTR_PORT_SPEED,
		.get	     = esw_get_port_speed,
	},
	{
		.type	     = SWITCH_TYPE_STRING,
		.name	     = "duplex",
		.description = "Get port duplex state",
		.id	     = RT5350_ESW_ATTR_PORT_DUPLEX,
		.get	     = esw_get_port_duplex,
	},
	{
		.type	     = SWITCH_TYPE_STRING,
		.name	     = "aneg",
		.description = "Get port auto negotiation status",
		.id	     = RT5350_ESW_ATTR_PORT_ANEG,
		.get	     = esw_get_port_auto_negotiation,
	},
	{
		.type	     = SWITCH_TYPE_STRING,
		.name	     = "advert",
		.description = "Get/set port speed advertisement, value range - 0x0-0xF",
		.id	     = RT5350_ESW_ATTR_PORT_ADVERT,
		.get	     = esw_get_port_speed_advertisement,
		.set	     = esw_set_port_speed_advertisement,
	},
};
...
static int esw_get_port_stats(struct switch_dev *dev, int port,
			 struct switch_port_stats *stats)
{
	struct rt305x_esw *esw = container_of(dev, struct rt305x_esw, swdev);

	if (port < 0 || port > RT305X_ESW_NUM_LEDS)
		return -EINVAL;

	// Assign shifted bad + good *packet* count into *byte* count: this isn't
	// completely horrible, because swconfig_led.c only checks for inequality
	// and no other code currently touches this.
	stats->rx_bytes = esw_r32(esw, RT305X_ESW_REG_RXPC(port));
	stats->tx_bytes = esw_r32(esw, RT305X_ESW_REG_TXPC(port));
...
static const struct switch_dev_ops esw_ops = {
	...
	.get_port_stats = esw_get_port_stats,
};
...
int esw_init(struct fe_priv *priv)
{
