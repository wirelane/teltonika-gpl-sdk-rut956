...
local FEED_PATH = "/etc/opkg/%s.conf"
local LIST_PATH = "/var/opkg-lists/%s"
local DIST_FEEDS = "/etc/opkg/openwrt/distfeeds.conf"
...
local function add_to_failed_packages(app_name, feed)
	local f = io.open(FAILED_PACKAGES, "a+")
	if f then
		local line = "%s - %s - %s" % { app_name, app_names_with_tlt_names[app_name], feed }
		f:write(line .. "\n")
		f:close()
	end
	fs.chmod(FAILED_PACKAGES, 660)
end
...
local dist_feeds_pkg_names = {}
local app_names = {}
for _, line in ipairs(util.split(pkg_text)) do
	DBG("Processing line: " .. line)
	local pkg_name, tlt_name, feed = line:match("^(.-) %- (.+) %- (.-)$")

	if pkg_name and tlt_name and feed then
		app_names[feed] = app_names[feed] or {}
		app_names_with_tlt_names[pkg_name] = tlt_name
		table.insert(app_names[feed], pkg_name)
		DBG("Preparing to install package: %s", pkg_name)
	else
		table.insert(dist_feeds_pkg_names, line)
		DBG("Preparing to install 3rd party package: %s", line)
	end
end
if not next(app_names) and #dist_feeds_pkg_names == 0 then
	util.perror("No packages to install")
	os.exit(0)
end


local function install_packages(feed)
	local retries = 0
	local pkg_list = {}
	while true do
		-- sometimes pkg list file doesn't appear instantly after opkg update - need to try a few times
		pkg_list = o_utils.get_pkg_list_by_app_name(false, feed)
		retries = retries + 1
	...
	local packages = {}
	for _, app_name in ipairs(app_names[feed] or {}) do
		util.perror("Validating package: " .. app_name)
		local pkg = OpkgPkg.get_available_pkg(app_name, false, feed)
		if pkg then
			...
			add_to_failed_packages(app_name, feed)
			...
			(function()
				...
				if not ok then
					if i == MAX_RETRIES then
						add_to_failed_packages(pkg.app_name, feed)
...
local feeds = {
	{ feed = "tlt_packages",    is_installed = uci:get("package_restore", "package_restore", "enabled") ~= "1",                 conf = "teltonikafeeds" },
	{ feed = "3rd_party_packages", is_installed = uci:get("package_restore", "package_restore", "3rd_party_packages_enabled") ~= "1", conf = "3rd_party_packages" }
}

while true do
	local is_complete = true
	for _, data in pairs(feeds) do
		if not app_names[data.feed] then
			data.is_installed = true
		end
		if not data.is_installed and o_utils.opkg_server_reachable(FEED_PATH % data.conf) then
			opkg_update { "--force_feeds", FEED_PATH % data.conf }
			if fs.access(LIST_PATH % data.feed) then
				local packages_stat = fs.stat(LIST_PATH % data.feed)
				local size = packages_stat and packages_stat.size or 0
				if size > 0 then
					install_packages(data.feed)
					data.is_installed = true
				end
			end
		end
		if not data.is_installed then is_complete = false end
	end
