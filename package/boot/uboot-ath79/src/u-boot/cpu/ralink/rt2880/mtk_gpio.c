...
#if defined(GPIO_POWER_LED_L)
	for (u32 i = 0; i < gpio_out_func_len; i++) {
		if (gpio_out_func[i].gpio & GPIO_POWER_LED_L) 
			gpio_set_level(i, 0);
	}
#endif // GPIO_POWER_LED_L
}
...
#ifdef CONFIG_GPIO_AIO_DIGITAL
	// If you need to controll MDI gpios
	RALINK_REG(MTK_SYSCTL_AGPIO_CFG) |= MTK_SYSCTL_AGPIO_CFG_GPIO_AIO_EN_MASK;
#endif // CONFIG_GPIO_AIO_DIGITAL
