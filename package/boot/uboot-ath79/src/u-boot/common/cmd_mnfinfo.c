...
#ifdef CONFIG_MNFINFO_LITE

        #define MNFINFO_PRINT(...) do {} while (0)

        #define MNFINFO_RESULT_OK(...) do {} while (0)

        #define MNFINFO_RESULT_ERR(...) printf("Internal error.\n")
...
static int mnf_cmd_get_field(const char *name, mnf_field_t **result)
{
	...
	if (!*result) {
		MNFINFO_RESULT_ERR("Invalid mnf field '%s'", name);
		return 1;
	}
...
inline static int mnf_cmd_read_field(mnf_field_t *field, char *buf)
{
...
inline static int mnf_cmd_write_init(void)
{
...
inline static int mnf_cmd_write_field(mnf_field_t *field, const char *data)
{
...
inline static int mnf_cmd_write_finalize(mnf_cmd_flags flags)
{
...
{
	static char old[256], new[256], oldf[256], newf[256];
...
inline static int do_list(int argc, char *const argv[])
{
	if (argc > 0)
...
inline static int do_show(int argc, char *const argv[])
{
	if (argc > 0)
...
inline static int do_get(int argc, char *const argv[])
{
	mnf_field_t *field;
	static char buf[256], formated[256];
...
inline static int do_set(int argc, char *const argv[])
{
	mnf_cmd_flags flags;
	mnf_field_t *field;
	static char buf[256];
...
inline static int do_repair(int argc, char *const argv[])
{
	mnf_cmd_flags flags;
...
inline static int do_restore(int argc, char *const argv[])
{
	mnf_cmd_flags flags;
	mnf_field_t *field;
...
inline int mnf_cmd_entry(int argc, char *argv[])
{
	const char *subcmd;
...
U_BOOT_CMD(mnf, 10, 0, do_mnfinfo, 
        "Edit mnf info\n",
	"<command> [args...]\n"
	"\n"
	"  Available commands:\n"
