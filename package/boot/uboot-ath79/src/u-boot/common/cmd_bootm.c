...
#ifdef CONFIG_TLT_VERIFIED_BOOT
#include "vboot.h"
#include "pubkey.h"
#define SHA256_NUMBYTES ((SHA256_DIGEST_SIZE) * 8)
int fw_verified = 0;
#endif
 ...
 */
static void print_tpl_ih_v1(tplink_image_header_t *hdr, u32 addr)
{
#ifdef CONFIG_TLT_VERIFIED_BOOT
...
#ifdef CONFIG_TLT_VERIFIED_BOOT
	if (ntohl(hdr->ih_rootfs_len) > 0 && ntohl(hdr->ih_rootfs_ofs) > 0) {
		data = (const uint8_t *)CFG_LOAD_ADDR;
		len = ntohl(hdr->ih_rootfs_ofs) +  ntohl(hdr->ih_rootfs_len) - SHA256_NUMBYTES;
	}
	else {
		data = (const uint8_t *)addr;
		len = ntohl(hdr->ih_kernel_ofs) +  ntohl(hdr->ih_kernel_len);
	}

	if (len > 0) {
		signature = data + len;

		if (!vboot_verify(data, (uint32_t)len, &pubkey, signature)) {
			fw_verified=1;
			puts("\n");
			printf("VerifiedBoot: Firmware verification successfull!\n");
		}
	}
#endif
	/* TODO: MD5 sum verify */
...
int do_bootm(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if(!fw_verified) {
		puts("\n");
		printf_err("VerifiedBoot: Firmware verification failed!\n");
		return 1;
	}
#endif
	...
	*/
	...
	case IH_COMP_LZ4:
		...
		/* Try to extract LZ4 data... */
		i = ulz4fn((u8 *)data, len,
